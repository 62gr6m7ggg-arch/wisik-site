...
test=test.replace(needle,"overflow:await activePage?.evaluate(()=>Array.from(document.querySelectorAll('body *')).map(e=>({tag:e.tagName,classes:e.className,rect:e.getBoundingClientRect().toJSON()})).filter(e=>e.rect.right>innerWidth+1||e.rect.left< -1).slice(0,25)).catch(()=>[]),document:await activePage?.content()");fs.writeFileSync(root+'test-browser.mjs',test);
