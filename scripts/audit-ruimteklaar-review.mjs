...
const report={version:'1.1-factor-gate',learnerVersion:fs.readFileSync('tools/space-tent/app/version.ts','utf8').match(/APP_VERSION='([^']+)'/)[1],status:'passed',auth,sourceFiles:Object.fromEntries(files.map(f=>[f,hash(fs.readFileSync(f))])),privateAssets:Object.fromEntries(Object.entries(assets).map(([f,a])=>[f,hash(a.body)])),browserEvidence:evidence};
