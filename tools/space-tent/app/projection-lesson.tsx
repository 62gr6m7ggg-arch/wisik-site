...
export function ProjectionLessonFrame({position}:{position:number}){
 ...
 return <div className="projection-lesson-frame" data-projection={b.kind}>
  ...
  <div className="insight-facts projection-measures" aria-label="Beeldlengtes in de voorbeeldtekening">{Object.entries(lengths).map(([name,value],i)=><span key={name} style={{color:COLORS[i]}}>{name}: {centimeter(value)} cm</span>)}</div>
