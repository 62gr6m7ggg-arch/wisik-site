...
function AngleImage({reveal}:{reveal:boolean}){
 return <><svg className="math-schema" viewBox="0 0 320 210" role="img" aria-label="Alleen het vlakke beeld is gegeven: de beelden van l en m staan loodrecht."><path d="M40 150 H285 M95 190 V25" fill="none" stroke="#ffa35e" strokeWidth="3"/><path d="M95 128 H117 V150" fill="none" stroke="#56d8cd" strokeWidth="2"/><text x="280" y="175">l</text><text x="110" y="35">m</text><text x="130" y="135">90°</text></svg>{reveal?<><div className="projection-pair"><div><h3>In de ruimte: 60°</h3><Geometry fixed compact highlights={['AF','AH']} caption="AF en AH zijn diagonalen van twee aangrenzende zijvlakken."/></div><div><h3>Van boven: 90°</h3><Geometry fixed compact view="top" highlights={['AF','AH']} caption="AF krijgt het beeld AB, AH het beeld AD: loodrechte beelden."/></div></div><p className="insight-explanation">AB en AD maken werkelijk 90° en hebben hetzelfde loodrechte bovenaanzicht. Uit alleen die beeldhoek volgt dus geen unieke ruimtelijke hoek.</p></>:<p className="figure-caption">Alleen de beeldhoek is gegeven. Er is geen ruimtelijke kubus bij deze vraag gegeven.</p>}</>
}
...
function RotatedCube({reveal}:{reveal:boolean}){
 return <><div className="projection-pair"><div><h3>Stand 1</h3><Geometry fixed compact frameBounds={[[-.87,-.87],[.87,.87]]} camera={[12,12]} highlights={['AD']}/></div><div><h3>Stand 2</h3><Geometry fixed compact frameBounds={[[-.87,-.87],[.87,.87]]} camera={[70,12]} highlights={['AD']}/></div></div><p className="figure-caption">{reveal?'De werkelijke kubus blijft dezelfde. Alleen de kijkrichting en daarmee de beeldlengte van AD verandert.':'Twee kijkrichtingen op dezelfde vaste kubus, bij gelijke schermschaal. AD wordt langer weergegeven.'}</p></>
}
function QuestionFigureContent({question:q,reveal=false,answer=[],selectable=[],onPoint,allowRotate=false,onExplore}:{allowRotate?:boolean;onExplore?:()=>void;question:Question;reveal?:boolean;answer?:string[];selectable?:string[];onPoint?:(point:string)=>void}){
...
export default function QuestionFigure(props:Parameters<typeof QuestionFigureContent>[0]){
 return <FigureGuidance.Provider value={!!props.allowRotate||!!props.reveal}><QuestionFigureContent key={props.question.id} {...props}/></FigureGuidance.Provider>;
}
