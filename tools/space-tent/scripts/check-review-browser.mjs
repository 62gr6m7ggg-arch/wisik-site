...
for(const config of [{name:'chromium-desktop',engine:chromium,width:1280,height:900,mobile:false},{name:'chromium-mobile',engine:chromium,width:390,height:844,mobile:true},{name:'webkit-mobile',engine:webkit,width:390,height:844,mobile:true}]){
 ...
 await page.getByLabel('Les of level').selectOption('7');await page.getByLabel('Soort onderdeel').selectOption('question');assert.equal(await page.getByLabel('Kies een onderdeel').locator('option').count(),29);await page.getByLabel('Vraagfunctie').selectOption('Eindproef B');assert.equal(await page.getByLabel('Kies een onderdeel').locator('option').count(),15);await page.getByRole('button',{name:'Filters wissen',exact:true}).click();
