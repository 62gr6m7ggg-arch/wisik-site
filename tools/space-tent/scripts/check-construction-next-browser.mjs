...
try{for(const [name,engine,width,mobile] of [['desktop',chromium,1280,false],['mobile',chromium,390,true],['webkit-mobile',webkit,390,true]]){
 ...
 for(const task of C.CONSTRUCTION_TASKS){await open(task);assert.equal(await page.getByRole('button',{name:'Volgende stap',exact:true}).count(),0);await page.getByRole('button',{name:'Constructie en onderbouwing inleveren',exact:true}).click();const next=page.getByRole('button',{name:'Volgende stap',exact:true});await next.waitFor();await page.waitForFunction(()=>[...document.querySelectorAll('button')].some(b=>b.textContent.includes('Volgende stap')&&!b.disabled));assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await next.click();await page.waitForFunction(()=>!document.querySelector('.workbench .success-panel'));const following=C.CONSTRUCTION_TASKS.find(t=>t.level===task.level&&C.CONSTRUCTION_TASKS.indexOf(t)>C.CONSTRUCTION_TASKS.indexOf(task));if(following)await page.getByRole('heading',{name:following.title,exact:true}).waitFor();else if(task.level<7)assert.equal(await page.locator('.workbench').count(),0);else await page.getByText('Alle stof, zelfstandig toegepast.',{exact:true}).waitFor();results.push({environment:name,task:task.id,status:'passed'});}
 const task=C.CONSTRUCTION_TASKS[0];await open(task,false);await page.getByRole('button',{name:'Constructie en onderbouwing inleveren',exact:true}).click();await page.getByRole('heading',{name:'Een volgende poging wordt gerichter.',exact:true}).waitFor();assert.equal(await page.getByRole('button',{name:'Volgende stap',exact:true}).count(),0);
 await open(task);await page.evaluate(()=>{Storage.prototype.setItem=()=>{throw new Error('storage full')}});await page.getByRole('button',{name:'Constructie en onderbouwing inleveren',exact:true}).click();assert.equal(await page.getByRole('button',{name:'Volgende stap',exact:true}).isDisabled(),true);
